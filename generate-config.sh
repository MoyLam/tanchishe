...
echo "🔧 开始生成 supabase-config.js 文件..."

# 检查 .env 文件是否存在
if [ ! -f ".env" ]; then
    echo "❌ 错误：.env 文件不存在"
    echo "请先复制 .env.example 为 .env 并填写您的配置"
    exit 1
fi
...
if [ -z "$SUPABASE_URL" ] || [ -z "$SUPABASE_ANON_KEY" ]; then
    echo "❌ 错误：.env 文件中缺少必要的配置"
    echo "请确保 SUPABASE_URL 和 SUPABASE_ANON_KEY 已正确配置"
    exit 1
fi

# 检查是否为示例值
if [[ "$SUPABASE_URL" == *"your_supabase"* ]] || [[ "$SUPABASE_ANON_KEY" == *"your_supabase"* ]]; then
    echo "❌ 错误：.env 文件中仍使用示例配置"
    echo "请填写真实的 Supabase 配置值"
    exit 1
fi
...
// Supabase配置文件
// 此文件由 generate-config.sh 自动生成，请勿手动编辑
...
echo "✅ supabase-config.js 文件生成成功！"
echo ""
echo "📋 配置信息:"
...
echo ""
echo "🚀 接下来可以:"
echo "  1. 在本地测试：直接打开 index.html"
echo "  2. 部署到服务器：将生成的 supabase-config.js 一起上传"
echo ""
echo "⚠️  注意：supabase-config.js 不会被提交到 Git（已在 .gitignore 中）"
